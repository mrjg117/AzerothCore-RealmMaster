...
show_build_header(){
  printf '\n%b\n' "${BLUE}🔨 AZEROTHCORE BUILD SYSTEM (PATCHED) 🔨${NC}"
  printf '%b\n' "${BLUE}═══════════════════════════════════${NC}"
...
apply_patches(){
  ...
  if [ ! -d "$patches_dir" ]; then
    info "No patches directory found, skipping patch application"
    return 0
  fi
  
  info "Applying custom patches from: $patches_dir"
  ...
  for patch_dir in "$patches_dir"/*/; do
    ...
    local module_name
    module_name=$(basename "$patch_dir")
    local target_dir="${src_path}/modules/${module_name}"
    
    if [ ! -d "$target_dir" ]; then
      warn "Module directory not found: $target_dir (skipping patch for $module_name)"
      continue
    fi
    
    info "Applying patch: $module_name"
    
    # 复制补丁文件到模块目录
    if cp -r "$patch_dir"/* "$target_dir/" 2>/dev/null; then
      ok "Patch applied: $module_name"
      patch_count=$((patch_count + 1))
    else
      err "Failed to apply patch: $module_name"
      return 1
    fi
  done
  
  if [ $patch_count -gt 0 ]; then
    ok "Successfully applied $patch_count patch(es)"
  else
    info "No patches were applied"
  fi
}
